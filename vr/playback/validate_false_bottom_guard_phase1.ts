...
type Mode = 'baseline' | 'guard_phase2' | 'guard_phase3' | 'guard_phase4'
...
const OUTPUT_DIR = join(process.cwd(), 'vr_backtest', 'results', 'false_bottom_guard_phase4')
const REPORT_PATH = join(process.cwd(), 'docs', 'vr_false_bottom_guard_phase4_report.md')
...
function analyzeVariant(
  ...
  const partialRows = executedRows.filter((row) => (row.point.trade_reason ?? '').includes('(delayed partial)'))
...
function buildEpisodeComparisons(view: NonNullable<ReturnType<typeof buildVRPlaybackView>>) {
  return TARGET_EPISODES
    .map((spec) => {
      ...
      const guardPhase4 = buildExecutionPlayback(event, INITIAL_CAP, {
        falseBottomGuard: true,
        guardReleaseProfile: 'phase4',
      }).variants[INITIAL_CAP]
      ...
      return {
        ...
        guard_phase4: analyzeVariant(event, guardPhase4, 'guard_phase4', spec.label),
...
function buildEpisodeSummaryRows(comparisons: EpisodeComparison[]) {
  return comparisons.map((episode) => ({
    ...
    phase4_final_value: episode.guard_phase4.final_value,
    ...
    phase4_final_value_delta: round(episode.guard_phase4.final_value - episode.baseline.final_value, 2),
    ...
    phase4_period_return_pct: episode.guard_phase4.period_return_pct,
    ...
    phase4_max_drawdown_pct: episode.guard_phase4.max_drawdown_pct,
    ...
    phase4_recovery_days: episode.guard_phase4.recovery_days,
    ...
    phase4_min_pool_balance: episode.guard_phase4.min_pool_balance,
    ...
    phase4_min_pool_balance_improvement: round(episode.guard_phase4.min_pool_balance - episode.baseline.min_pool_balance, 2),
    ...
    phase4_pool_depletion_count: episode.guard_phase4.pool_depletion_count,
    ...
    phase4_pool_near_zero_days: episode.guard_phase4.pool_near_zero_days,
    ...
    phase4_pool_usage_rate_pct: episode.guard_phase4.pool_usage_rate_pct,
    ...
    phase4_episode_max_pool_stress_pct: episode.guard_phase4.episode_max_pool_stress_pct,
    ...
    phase4_miss_cost_5d_pct: episode.guard_phase4.average_guarded_forward_return_5d_pct,
    phase4_miss_cost_10d_pct: episode.guard_phase4.average_guarded_forward_return_10d_pct,
    phase4_miss_cost_20d_pct: episode.guard_phase4.average_guarded_forward_return_20d_pct,
  }))
}
...
function buildTradeBehaviorRows(comparisons: EpisodeComparison[]) {
  return comparisons.map((episode) => ({
    ...
    phase4_total_vmin_trigger_events: episode.guard_phase4.total_vmin_trigger_events,
    ...
    phase4_executed_vmin_buys: episode.guard_phase4.executed_vmin_buys,
    ...
    phase4_partial_buys_count: episode.guard_phase4.partial_buys_count,
    ...
    phase4_delayed_buys_count: episode.guard_phase4.delayed_buys_count,
    ...
    phase4_blocked_buys_count: episode.guard_phase4.blocked_buys_count,
    ...
    phase4_average_delay_bars: episode.guard_phase4.average_delay_bars,
    ...
    phase4_delayed_fill_rate_pct: episode.guard_phase4.delayed_fill_rate_pct,
    ...
    phase4_blocked_then_lower_price_reentry_rate_pct: episode.guard_phase4.blocked_then_lower_price_reentry_rate_pct,
  }))
}
...
function buildGuardDiagnosticsRows(comparisons: EpisodeComparison[]) {
  return comparisons.map((episode) => ({
    ...
    phase4_guard_active_days: episode.guard_phase4.guard_active_days,
    ...
    phase4_guard_reset_count: episode.guard_phase4.guard_reset_count,
    ...
    phase4_avg_days_to_first_vmin_buy_after_reset: episode.guard_phase4.avg_days_to_first_vmin_buy_after_reset,
    ...
    phase4_avg_reset_first_buy_10d_return_pct: episode.guard_phase4.avg_reset_first_buy_10d_return_pct,
    ...
    phase4_reset_delayed_missed_rebound_cases: episode.guard_phase4.reset_delayed_missed_rebound_cases,
    ...
    phase4_reset_false_positive_rate_pct: episode.guard_phase4.reset_false_positive_rate_pct,
  }))
}
...
function buildReportMarkdown(comparisons: EpisodeComparison[]) {
  ...
  const strongestPhase4PoolRelief = [...summaryRows]
    .sort((left, right) => Number(right.phase4_min_pool_balance_improvement ?? 0) - Number(left.phase4_min_pool_balance_improvement ?? 0))[0]
  const largestPhase4MissCost = [...summaryRows]
    .filter((row) => typeof row.phase4_miss_cost_10d_pct === 'number')
    .sort((left, right) => Number(right.phase4_miss_cost_10d_pct ?? -999) - Number(left.phase4_miss_cost_10d_pct ?? -999))[0]

  return `# VR False-Bottom Guard Phase 4 Report
...
${markdownTable(summaryRows.map((row) => ({
    ...
    phase4_dd: row.phase4_max_drawdown_pct as number,
    ...
    phase4_min_pool: row.phase4_min_pool_balance as number,
    ...
    phase4_pool_relief: row.phase4_min_pool_balance_improvement as number,
    ...
    phase4_near_zero_days: row.phase4_pool_near_zero_days as number,
  })))}
...
${markdownTable(behaviorRows.map((row) => ({
    ...
    phase4_avg_delay_bars: row.phase4_average_delay_bars as number | null,
  })))}
...
${markdownTable(diagnosticsRows.map((row) => ({
    ...
    phase4_guard_active_days: row.phase4_guard_active_days as number,
    phase2_resets: row.phase2_guard_reset_count as number,
    phase3_resets: row.phase3_guard_reset_count as number,
    phase4_resets: row.phase4_guard_reset_count as number,
    ...
    phase4_avg_days_to_first_buy_after_reset: row.phase4_avg_days_to_first_vmin_buy_after_reset as number | null,
    ...
    phase4_reset_missed_rebound_cases: row.phase4_reset_delayed_missed_rebound_cases as number,
    ...
    phase4_reset_false_positive_rate_pct: row.phase4_reset_false_positive_rate_pct as number | null,
  })))}
...
${markdownTable(summaryRows
  ...
  .map((row) => ({
    ...
    phase4_miss_cost_10d_pct: row.phase4_miss_cost_10d_pct as number | null,
    ...
    phase4_reset_false_positive_rate_pct: diagnosticsRows.find((item) => item.event_id === row.event_id)?.phase4_reset_false_positive_rate_pct as number | null,
  })))}

## Key findings
- Largest Phase 4 pool relief: ${strongestPhase4PoolRelief ? `${strongestPhase4PoolRelief.event_label} (${strongestPhase4PoolRelief.phase4_min_pool_balance_improvement} improvement in minimum pool balance)` : 'Not available'}
- Largest Phase 4 miss cost (10d average from blocked/delayed signals): ${largestPhase4MissCost ? `${largestPhase4MissCost.event_label} (${largestPhase4MissCost.phase4_miss_cost_10d_pct}%)` : 'Not available'}
...
function main() {
  ...
  const phase4GuardSignals = comparisons.flatMap((episode) =>
    episode.guard_phase4.guard_signal_events.map((row) => ({
      mode: 'guard_phase4',
      ...row,
    })),
  )
  const guardSignals = [...phase2GuardSignals, ...phase3GuardSignals, ...phase4GuardSignals]
  ...
  const phase4GuardTimelineRows = comparisons.flatMap((episode) =>
    episode.guard_phase4.guard_timeline.map((row) => ({
      event_id: episode.event_id,
      event_label: episode.event_label,
      mode: 'guard_phase4',
      ...row,
  ...
  const guardTimelineRows = [...phase2GuardTimelineRows, ...phase3GuardTimelineRows, ...phase4GuardTimelineRows]
