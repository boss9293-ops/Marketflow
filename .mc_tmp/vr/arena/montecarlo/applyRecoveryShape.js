...
exports.applyRecoveryShape = applyRecoveryShape;
const SHAPE_END_RECOVERY = {
...
function getRecoveryProgress(args) {
    ...
    switch (shape) {
        ...
        case 'GRINDING_BEAR':
            return Math.max(0, endRecovery * u + Math.sin(u * Math.PI * 4) * endRecovery * 0.12);
...
function applyRecoveryShape(args) {
    ...
    if (recoveryStart >= recoveryEnd)
        return nextReturns;
    const crashMultiplier = args.crashReturns.reduce((value, dailyReturn) => value * (1 + dailyReturn), 1);
    ...
    for (let index = recoveryStart; index < recoveryEnd; index += 1) {
        const progress = recoveryEnd - recoveryStart <= 1
            ? 1
        ...
        const targetPrice = crashMultiplier + (1 - crashMultiplier) * recoveryProgress;
        const shapedReturn = previousSyntheticPrice > 0
            ? targetPrice / previousSyntheticPrice - 1
            : 0;
        const baseNoise = baseRecoveryReturns[index - recoveryStart] ?? 0;
        const adjustedReturn = clampReturn(shapedReturn + baseNoise * NOISE_SCALE[args.recoveryShape]);
