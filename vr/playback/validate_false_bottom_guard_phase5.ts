...
type Mode = 'baseline' | 'guard_phase2' | 'guard_phase3' | 'guard_phase5'
...
type VariantMetrics = {
  ...
  fast_snapback_flag_days: number
  override_weak_days: number
  override_moderate_days: number
  fast_snapback_override_buy_count: number
...
const OUTPUT_DIR = join(process.cwd(), 'vr_backtest', 'results', 'false_bottom_guard_phase5')
const REPORT_PATH = join(process.cwd(), 'docs', 'vr_false_bottom_guard_phase5_report.md')
...
function analyzeVariant(
  ...
  const fastSnapbackFlagDays = rows.filter((row) => row.point.fast_snapback_flag).length
  const overrideWeakDays = rows.filter((row) => row.point.override_strength === 'WEAK').length
  const overrideModerateDays = rows.filter((row) => row.point.override_strength === 'MODERATE').length
  const fastSnapbackOverrideBuyCount = executedRows.filter((row) =>
    (row.point.trade_reason ?? '').includes('fast snapback'),
  ).length
  ...
  return {
    ...
    fast_snapback_flag_days: fastSnapbackFlagDays,
    override_weak_days: overrideWeakDays,
    override_moderate_days: overrideModerateDays,
    fast_snapback_override_buy_count: fastSnapbackOverrideBuyCount,
...
function buildEpisodeComparisons(view: NonNullable<ReturnType<typeof buildVRPlaybackView>>) {
  return TARGET_EPISODES
    .map((spec) => {
      ...
      const guardPhase5 = buildExecutionPlayback(event, INITIAL_CAP, {
        falseBottomGuard: true,
      ...
      return {
        ...
        guard_phase5: analyzeVariant(event, guardPhase5, 'guard_phase5', spec.label),
...
function buildEpisodeSummaryRows(comparisons: EpisodeComparison[]) {
  return comparisons.map((episode) => ({
    ...
    phase5_final_value: episode.guard_phase5.final_value,
    ...
    phase5_final_value_delta: round(episode.guard_phase5.final_value - episode.baseline.final_value, 2),
    ...
    phase5_period_return_pct: episode.guard_phase5.period_return_pct,
    ...
    phase5_max_drawdown_pct: episode.guard_phase5.max_drawdown_pct,
    ...
    phase5_recovery_days: episode.guard_phase5.recovery_days,
    ...
    phase5_min_pool_balance: episode.guard_phase5.min_pool_balance,
    ...
    phase5_min_pool_balance_improvement: round(episode.guard_phase5.min_pool_balance - episode.baseline.min_pool_balance, 2),
    ...
    phase5_pool_depletion_count: episode.guard_phase5.pool_depletion_count,
    ...
    phase5_pool_near_zero_days: episode.guard_phase5.pool_near_zero_days,
    ...
    phase5_pool_usage_rate_pct: episode.guard_phase5.pool_usage_rate_pct,
    ...
    phase5_episode_max_pool_stress_pct: episode.guard_phase5.episode_max_pool_stress_pct,
    ...
    phase5_miss_cost_5d_pct: episode.guard_phase5.average_guarded_forward_return_5d_pct,
    phase5_miss_cost_10d_pct: episode.guard_phase5.average_guarded_forward_return_10d_pct,
    phase5_miss_cost_20d_pct: episode.guard_phase5.average_guarded_forward_return_20d_pct,
  }))
}
...
function buildTradeBehaviorRows(comparisons: EpisodeComparison[]) {
  return comparisons.map((episode) => ({
    ...
    phase5_total_vmin_trigger_events: episode.guard_phase5.total_vmin_trigger_events,
    ...
    phase5_executed_vmin_buys: episode.guard_phase5.executed_vmin_buys,
    ...
    phase5_partial_buys_count: episode.guard_phase5.partial_buys_count,
    ...
    phase5_delayed_buys_count: episode.guard_phase5.delayed_buys_count,
    ...
    phase5_blocked_buys_count: episode.guard_phase5.blocked_buys_count,
    ...
    phase5_average_delay_bars: episode.guard_phase5.average_delay_bars,
    ...
    phase5_delayed_fill_rate_pct: episode.guard_phase5.delayed_fill_rate_pct,
    ...
    phase5_blocked_then_lower_price_reentry_rate_pct: episode.guard_phase5.blocked_then_lower_price_reentry_rate_pct,
  }))
}
...
function buildGuardDiagnosticsRows(comparisons: EpisodeComparison[]) {
  return comparisons.map((episode) => ({
    ...
    phase5_guard_active_days: episode.guard_phase5.guard_active_days,
    ...
    phase5_guard_reset_count: episode.guard_phase5.guard_reset_count,
    ...
    phase5_avg_days_to_first_vmin_buy_after_reset: episode.guard_phase5.avg_days_to_first_vmin_buy_after_reset,
    ...
    phase5_avg_reset_first_buy_10d_return_pct: episode.guard_phase5.avg_reset_first_buy_10d_return_pct,
    ...
    phase5_reset_delayed_missed_rebound_cases: episode.guard_phase5.reset_delayed_missed_rebound_cases,
    ...
    phase5_reset_false_positive_rate_pct: episode.guard_phase5.reset_false_positive_rate_pct,
  }))
}
...
function buildFastSnapbackOverrideRows(comparisons: EpisodeComparison[]) {
  return comparisons.map((episode) => ({
    event_id: episode.event_id,
    event_label: episode.event_label,
    phase5_fast_snapback_flag_days: episode.guard_phase5.fast_snapback_flag_days,
    phase5_override_weak_days: episode.guard_phase5.override_weak_days,
    phase5_override_moderate_days: episode.guard_phase5.override_moderate_days,
    phase5_fast_snapback_override_buy_count: episode.guard_phase5.fast_snapback_override_buy_count,
  }))
}
...
function buildReportMarkdown(comparisons: EpisodeComparison[]) {
  ...
  const strongestPhase5PoolRelief = [...summaryRows]
    .sort((left, right) => Number(right.phase5_min_pool_balance_improvement ?? 0) - Number(left.phase5_min_pool_balance_improvement ?? 0))[0]
  const largestPhase5MissCost = [...summaryRows]
    .filter((row) => typeof row.phase5_miss_cost_10d_pct === 'number')
    .sort((left, right) => Number(right.phase5_miss_cost_10d_pct ?? -999) - Number(left.phase5_miss_cost_10d_pct ?? -999))[0]

  return `# VR False-Bottom Guard Phase 5 Report
...
${markdownTable(summaryRows.map((row) => ({
    ...
    phase5_dd: row.phase5_max_drawdown_pct as number,
    ...
    phase5_min_pool: row.phase5_min_pool_balance as number,
    ...
    phase5_pool_relief: row.phase5_min_pool_balance_improvement as number,
    ...
    phase5_near_zero_days: row.phase5_pool_near_zero_days as number,
  })))}
...
${markdownTable(behaviorRows.map((row) => ({
    ...
    phase5_avg_delay_bars: row.phase5_average_delay_bars as number | null,
  })))}
...
${markdownTable(diagnosticsRows.map((row) => ({
    ...
    phase5_guard_active_days: row.phase5_guard_active_days as number,
    phase2_resets: row.phase2_guard_reset_count as number,
    phase3_resets: row.phase3_guard_reset_count as number,
    phase5_resets: row.phase5_guard_reset_count as number,
    ...
    phase5_avg_days_to_first_buy_after_reset: row.phase5_avg_days_to_first_vmin_buy_after_reset as number | null,
    ...
    phase5_reset_missed_rebound_cases: row.phase5_reset_delayed_missed_rebound_cases as number,
    ...
    phase5_reset_false_positive_rate_pct: row.phase5_reset_false_positive_rate_pct as number | null,
  })))}

### Fast snapback override diagnostics
${markdownTable(overrideRows.map((row) => ({
    event: row.event_label as string,
    phase5_fast_snapback_flag_days: row.phase5_fast_snapback_flag_days as number,
    phase5_override_weak_days: row.phase5_override_weak_days as number,
    phase5_override_moderate_days: row.phase5_override_moderate_days as number,
    phase5_fast_snapback_override_buy_count: row.phase5_fast_snapback_override_buy_count as number,
  })))}
...
${markdownTable(summaryRows
  ...
  .map((row) => ({
    ...
    phase5_miss_cost_10d_pct: row.phase5_miss_cost_10d_pct as number | null,
    ...
    phase5_reset_false_positive_rate_pct: diagnosticsRows.find((item) => item.event_id === row.event_id)?.phase5_reset_false_positive_rate_pct as number | null,
  })))}

## Key findings
- Largest Phase 5 pool relief: ${strongestPhase5PoolRelief ? `${strongestPhase5PoolRelief.event_label} (${strongestPhase5PoolRelief.phase5_min_pool_balance_improvement} improvement in minimum pool balance)` : 'Not available'}
- Largest Phase 5 miss cost (10d average from blocked/delayed signals): ${largestPhase5MissCost ? `${largestPhase5MissCost.event_label} (${largestPhase5MissCost.phase5_miss_cost_10d_pct}%)` : 'Not available'}
...
function main() {
  ...
  const phase5GuardSignals = comparisons.flatMap((episode) =>
    episode.guard_phase5.guard_signal_events.map((row) => ({
      mode: 'guard_phase5',
      ...row,
    })),
  )
  const guardSignals = [...phase2GuardSignals, ...phase3GuardSignals, ...phase5GuardSignals]
  ...
  const phase5GuardTimelineRows = comparisons.flatMap((episode) =>
    episode.guard_phase5.guard_timeline.map((row) => ({
      event_id: episode.event_id,
      event_label: episode.event_label,
      mode: 'guard_phase5',
      ...row,
  ...
  const guardTimelineRows = [...phase2GuardTimelineRows, ...phase3GuardTimelineRows, ...phase5GuardTimelineRows]
