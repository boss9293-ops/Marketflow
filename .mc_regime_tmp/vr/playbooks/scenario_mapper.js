...
exports.mapPatternToScenarios = mapPatternToScenarios;
const fs_1 = require("fs");
const path_1 = require("path");
function loadPatternSeed(rootDir, patternId) {
    try {
        const filePath = (0, path_1.join)(rootDir, 'vr', 'patterns', `${patternId}.json`);
...
function inferDescription(name) {
    ...
    if (key.includes('range_continuation'))
        return 'The market may continue to move sideways while reacting to headlines.';
    if (key.includes('support_breakdown'))
        return 'Selling pressure remains active and recent support is under pressure.';
    if (key.includes('relief_rally_breakout'))
        return 'A relief rally may emerge, but failed follow-through remains a risk.';
    if (key.includes('relief_rally'))
        return 'A rebound is possible, but confirmation remains limited.';
    if (key.includes('recovery'))
        return 'A rebound is possible, but confirmation remains limited.';
    if (key.includes('dead_cat_bounce'))
        return 'A rebound may occur, but lower-high failure risk remains elevated.';
    if (key.includes('extended_correction'))
        return 'Weakness may persist in a broader corrective structure.';
    if (key.includes('panic_bottom'))
        return 'Selling may exhaust quickly, but instability remains high.';
    if (key.includes('secondary_crash'))
        return 'A second downside leg remains plausible if support fails again.';
    if (key.includes('extended_bear_move'))
        return 'The correction may deepen into a more persistent bear phase.';
    if (key.includes('range'))
        return 'The market may continue moving sideways while direction remains unresolved.';
    if (key.includes('breakdown'))
        return 'The lower boundary of the structure fails and downside pressure resumes.';
    if (key.includes('breakout'))
        return 'A rebound may push through resistance, but confirmation remains limited.';
    if (key.includes('bounce'))
        return 'A rebound may occur, but failed follow-through remains a risk.';
    return 'This branch represents a plausible next path for the current market structure.';
}
function inferPostureGuidance(name, explicitPosture) {
    if (explicitPosture) {
...
function mapPlaybookToZoneActions(playbook) {
    if (!playbook)
        return undefined;
    const zoneActions = {
...
function buildFallbackScenarios(match, seed) {
    ...
    return scenarioSeeds
        .slice(0, 3)
        .map((scenario, index) => {
        const rawName = typeof scenario === 'string'
            ? scenario
            : scenario.title ?? scenario.id ?? `scenario_${index + 1}`;
        return {
            scenario_id: typeof scenario === 'string'
                ? scenario
            ...
            description: typeof scenario === 'string'
                ? inferDescription(scenario)
                : scenario.description ?? inferDescription(rawName),
            posture_guidance: typeof scenario === 'string'
...
function mapPatternToScenarios(match, options) {
    ...
    if (Array.isArray(interpretationBranches) && interpretationBranches.length > 0) {
        return interpretationBranches
            ...
            posture_guidance: Array.isArray(branch.posture_guidance) && branch.posture_guidance.length > 0
