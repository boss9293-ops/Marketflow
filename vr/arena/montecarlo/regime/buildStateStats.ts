import type { RegimeLabelPoint, RegimeState, RegimeStateStats } from './types'
...
const FALLBACK_STATS: Record<RegimeState, RegimeStateStats> = {
  NORMAL: {
    state: 'NORMAL',
    meanReturn: 0.0007,
    volReturn: 0.012,
...
function segmentDurations(labels: RegimeLabelPoint[]) {
  const durations = new Map<RegimeState, number[]>()
  for (const state of REGIME_STATES) {
    durations.set(state, [])
  }
  if (!labels.length) return durations
...
export function buildStateStats(args: {
  returns: number[]
  labels: RegimeLabelPoint[]
}): Record<RegimeState, RegimeStateStats> {
  const returnsByState = new Map<RegimeState, number[]>()
  for (const state of REGIME_STATES) {
  ...
  const stats = {} as Record<RegimeState, RegimeStateStats>
