...
function inferProxyScenarioHint(prices: number[]) {
  if (prices.length < 2) return 'Mixed' as WarningScenarioHint
  ...
  if (anchorIndex < 0) return 'Mixed'

  const window = prices.slice(anchorIndex)
  const duration = window.length
  const bottomIndex = window.reduce(
    (best, point, index, array) => (point < array[best] ? index : best),
    0
  )
  const peakBeforeBottom = window
    .slice(0, bottomIndex + 1)
    .reduce((best, point) => Math.max(best, point), window[0])
  const recoveryIndex = window.findIndex(
    (point, index) => index > bottomIndex && point >= peakBeforeBottom * 0.95
  )
  const reboundBars = recoveryIndex >= 0 ? recoveryIndex - bottomIndex : Number.POSITIVE_INFINITY

  if (duration <= 60 && reboundBars <= 20) return 'V'
  if (duration > 120) return 'Bear'
  if (duration <= 120) return 'Correction'
  return 'Mixed'
}
...
export function extractWarningTrace(args: {
  pathId: string
  prices: number[]
  injectedScenarioMeta?: InjectedScenarioMeta | null
}): WarningTracePoint[] {
  if (!args.prices.length) return []
  ...
  for (let index = 1; index < args.prices.length; index += 1) {
    ...
    const distanceToMa =
      typeof ma200[index] === 'number' && ma200[index]! > 0
    ...
    const reboundFromLow =
      trackedLow != null && trackedLow > 0 ? current / trackedLow - 1 : null
