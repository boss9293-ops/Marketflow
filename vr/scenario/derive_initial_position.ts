...
export function deriveInitialPosition(input: {
  startPrice: number
  overrides?: EventInitialStateOverrides
}): EventInitialState {
  const overrides = input.overrides ?? {}
  const initialCapital = overrides.initial_capital ?? EVENT_INITIAL_STATE_DEFAULTS.initialCapital
  const stockAllocationPct = overrides.stock_allocation_pct ?? EVENT_INITIAL_STATE_DEFAULTS.stockAllocationPct
  const poolAllocationPct = overrides.pool_allocation_pct ?? EVENT_INITIAL_STATE_DEFAULTS.poolAllocationPct
