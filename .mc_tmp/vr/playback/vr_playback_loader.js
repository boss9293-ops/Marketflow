...
exports.buildVRPlaybackView = buildVRPlaybackView;
exports.runVRPlaybackReadinessExamples = runVRPlaybackReadinessExamples;
const pattern_detector_1 = require("../../engine/pattern_detector");
const build_execution_playback_1 = require("./build_execution_playback");
const build_event_cycle_framework_1 = require("./build_event_cycle_framework");
const playbook_mapper_1 = require("../playbooks/playbook_mapper");
const initialize_event_state_1 = require("../scenario/initialize_event_state");
const vr_event_interpreter_1 = require("./vr_event_interpreter");
const CURATED_PLAYBACK_SUITE = [
    {
...
function buildSyntheticTqqqProxy(points) {
    ...
    return points.map((point, index) => {
        ...
        const tqqqDd = syntheticPeak && syntheticPeak > 0
...
function computeReboundStrength(event) {
    const points = event.playback.filter((point) => point.in_ev && typeof point.qqq_n === 'number');
    if (!points.length)
        return null;
    const trough = Math.min(...points.map((point) => point.qqq_n));
    const last = points[points.length - 1]?.qqq_n;
    if (typeof last !== 'number' || trough <= 0)
        return null;
    return Number((((last - trough) / trough) * 100).toFixed(1));
}
function computeRecoveryPath(event) {
    const reboundStrength = computeReboundStrength(event);
    return {
        rebound_strength_pct: reboundStrength,
        rebound_persistence: reboundStrength == null ? 'unavailable'
            : reboundStrength >= 12 ? 'improving'
                : reboundStrength >= 5 ? 'mixed'
                    : 'weak',
        lower_high_failure_risk: reboundStrength == null ? 'unavailable'
            : reboundStrength >= 12 ? 'medium'
                : 'high',
        secondary_drawdown_risk: Math.min(...event.playback.map((point) => point.dd ?? 0)) <= -15 ? 'high' : 'medium',
    };
}
function inferDetectorInput(event, syntheticTqqqDrawdownPct) {
    ...
    const tqqqDrawdown = tqqqSeries.length
        ? Math.min(...tqqqSeries) / 100
        : typeof syntheticTqqqDrawdownPct === 'number'
            ? syntheticTqqqDrawdownPct / 100
            : qqqDrawdown * 3;
    const breachPoints = event.playback.filter((point) => typeof point.qqq_n === 'number' &&
        typeof point.ma200_n === 'number' &&
        point.qqq_n < point.ma200_n).length;
    return {
        ...
        volatility_regime: Math.abs(qqqDrawdown) >= 0.18 ? 'extreme'
            : Math.abs(qqqDrawdown) >= 0.1 ? 'high'
                : Math.abs(qqqDrawdown) >= 0.06 ? 'elevated'
                    : 'moderate',
        price_structure: durationDays <= 15 && Math.abs(qqqDrawdown) >= 0.15 ? 'vertical_drop'
            : durationDays >= 35 && Math.abs(qqqDrawdown) >= 0.1 ? 'slow_bleed'
                : Math.abs(qqqDrawdown) <= 0.12 && durationDays >= 20 ? 'range_market'
                    : breachPoints > 0 ? 'breakdown_retest'
                        : 'trend_down',
        catalyst_type: event.name.includes('2026-02') || event.name.includes('2024-07')
            ? 'geopolitical_event'
            : event.name.includes('2020-02')
                ? 'volatility_spike'
                : undefined,
        rebound_behavior: computeReboundStrength(event) == null ? 'none'
            : computeReboundStrength(event) >= 12 ? 'strong'
                : computeReboundStrength(event) >= 5 ? 'mixed'
                    : 'weak',
        trend_persistence: durationDays >= 30 && Math.abs(qqqDrawdown) >= 0.1 ? 'down'
            : durationDays >= 20 ? 'sideways'
                : 'up',
    };
}
function buildVRPlaybackView(input) {
    ...
    const rawEventViews = input.standardArchive.events.map((event) => {
        ...
        const qqqDrawdownPct = chartData.reduce((min, point) => {
            if (typeof point.qqq_dd !== 'number')
                return min;
            return min == null ? point.qqq_dd : Math.min(min, point.qqq_dd);
        }, null);
        const tqqqDrawdownPct = chartData.reduce((min, point) => {
            if (typeof point.tqqq_dd !== 'number')
                return min;
            return min == null ? point.tqqq_dd : Math.min(min, point.tqqq_dd);
        }, null);
        const detectorInput = inferDetectorInput(event, tqqqDrawdownPct);
        const patternMatches = (0, pattern_detector_1.detectPatternMatches)(detectorInput, { rootDir: input.rootDir, limit: 3 });
        const scenarioPlaybook = (0, playbook_mapper_1.mapScenarioPlaybook)(patternMatches, { rootDir: input.rootDir, maxScenarios: 3 });
        const ma200Status = detectorInput.ma200_relation === 'below' ? 'Sustained Below MA200'
        ...
        const vrTaggedEvent = (0, vr_event_interpreter_1.resolveVREventInterpretation)({
        ...
        const cycleStart = (0, initialize_event_state_1.initializeEventState)({
            rootDir: input.rootDir,
            eventId,
        ...
        const placeholderMessages = supportStatus === 'pending_synthetic'
            ? [
        ...
        const baseEventView = {
            ...
            leveraged_stress: {
                ...
                amplification: qqqDrawdownPct != null && tqqqDrawdownPct != null && qqqDrawdownPct !== 0
        ...
        const eventView = {
            ...baseEventView,
            cycle_framework: (0, build_event_cycle_framework_1.buildEventCycleFramework)({ event: baseEventView }),
        };
        return {
            ...eventView,
            execution_playback: (0, build_execution_playback_1.buildExecutionPlayback)(eventView),
        };
    });
...
function runVRPlaybackReadinessExamples(view) {
    const cases = [
    ...
    return cases.map((testCase) => {
        ...
        return {
            name: testCase.name,
            passed: event?.vr_support_status === testCase.expected &&
