...
const buildFeatureVector_1 = require("./montecarlo/overlay/buildFeatureVector");
const computeMonteCarloOverlay_1 = require("./montecarlo/overlay/computeMonteCarloOverlay");
const findSimilarMcPaths_1 = require("./montecarlo/overlay/findSimilarMcPaths");
const ARENA_TARGETS = [
...
let cachedMcFingerprintLibrary;
function findLatestFingerprintLibraryFile(rootDir) {
    ...
    while (stack.length) {
        ...
        if (!currentDir)
            continue;
        for (const entry of (0, fs_1.readdirSync)(currentDir, { withFileTypes: true })) {
            const fullPath = (0, path_1.join)(currentDir, entry.name);
            if (entry.isDirectory()) {
                stack.push(fullPath);
                continue;
            }
            if (!entry.isFile() || entry.name !== 'arena_mc_fingerprint_library.json') {
                continue;
            }
...
function loadMonteCarloFingerprintLibrary() {
    ...
    try {
        const rootDir = (0, path_1.join)(process.cwd(), 'marketflow_data', 'arena_mc');
        const libraryPath = findLatestFingerprintLibraryFile(rootDir);
        if (!libraryPath) {
            cachedMcFingerprintLibrary = null;
            return cachedMcFingerprintLibrary;
        }
        const raw = (0, fs_1.readFileSync)(libraryPath, 'utf-8');
...
function buildMonteCarloOverlayForWarningLayer(warningLayer) {
    ...
    return similarPaths.length ? (0, computeMonteCarloOverlay_1.computeMonteCarloOverlay)({ current, similarPaths }) : null;
}
    ...
    for (let index = 1; index < assetSeries.length; index += 1) {
        ...
        if (current.date >= visibleStartDate && current.date <= visibleEndDate) {
            visibleSnapshots.push({
                ...
                rebound_from_low_pct: reboundFromLow != null ? Number((reboundFromLow * 100).toFixed(2)) : null,
