...
exports.getSmartAnalyzerPrompt = getSmartAnalyzerPrompt;
...
function countDistinctMatches(text, keywords) {
...
function hasReboundSignal(input) {
...
function scorePersistence(input) {
    const reasons = [];
...
function scoreLiquidity(input) {
    ...
    if (typeof liquidity.rrp === 'number' &&
...
function buildScoreState(macro, persistence, reaction, event, velocity, liquidity, credit, internals) {
    return {
...
function resolveBaseMarketType(scoreState) {
...
function applyVelocityAdjustment(baseMarketType, scoreState) {
    const reasons = [];
...
function applyMacroExpansionAdjustment(currentMarketType, scoreState) {
    const reasons = [];
    let marketType = currentMarketType;
    if (currentMarketType === 'HYBRID' &&
...
function buildClassificationReason(input, scoreState, baseMarketType, velocityAdjustedMarketType, finalMarketType, velocity, liquidity, credit, internals, adjustmentReasons) {
    const reasons = [];
...
function resolveConfidence(marketType, scoreState) {
...
function resolveStrategy(marketType, scoreState) {
    let strategy;
    if (marketType === 'STRUCTURAL') {
        strategy =
...
function buildKeyDrivers(marketType, scoreState, input) {
    const drivers = [];
    ...
    const orderedDrivers = marketType === 'STRUCTURAL'
        ? structuralDrivers
        : marketType === 'EVENT'
            ? eventDrivers
            : hybridDrivers;
...
function buildInterpretation(marketType, scoreState) {
    const speedSentence = scoreState.velocity_score >= 60
        ? '하락 속도도 비정상적으로 빨라 단순 헤드라인 변동으로만 보기 어렵습니다.'
        : '';
    const liquidityCreditSentence = scoreState.liquidity_score >= 60 && scoreState.credit_score >= 60
        ...
        : scoreState.liquidity_score >= 60
            ...
            : scoreState.credit_score >= 60
                ...
                : '';
    const internalsSentence = scoreState.internals_score >= 60
    ...
    const balanceSentence = scoreState.macro_score >= scoreState.event_score
...
function buildSummary(marketType, strategy, scoreState) {
    if (scoreState.velocity_score >= 80) {
...
function getSmartAnalyzerPrompt() {
    return (0, promptLoader_1.loadEnginePrompt)(SMART_ANALYZER_PROMPT_FILE);
}
function analyzeMarket(input) {
    ...
    const scoreState = buildScoreState(macro, persistence, reaction, event, velocity, liquidity, credit, internals);
    ...
    const debug = {
        ...scoreState,
        classification_reason: buildClassificationReason(input, scoreState, baseMarketType, velocityAdjusted.marketType, finalMarketType, velocity, liquidity, credit, internals, [...velocityAdjusted.reasons, ...expansionAdjusted.reasons]),
    };
    const confidence = resolveConfidence(finalMarketType, scoreState);
    const strategy = resolveStrategy(finalMarketType, scoreState);
    const result = {
        market_type: finalMarketType,
        confidence,
        key_drivers: buildKeyDrivers(finalMarketType, scoreState, input),
        interpretation: buildInterpretation(finalMarketType, scoreState),
        strategy,
        summary: buildSummary(finalMarketType, strategy, scoreState),
        debug,
    };
    if (shouldLog()) {
        console.info('[smartAnalyzer]', JSON.stringify({
            input,
            result,
        }, null, 2));
    }
