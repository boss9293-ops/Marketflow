...
exports.attachPriorityEventVRTag = attachPriorityEventVRTag;
exports.runPriorityEventTagExamples = runPriorityEventTagExamples;
const load_priority_event_tags_1 = require("./load_priority_event_tags");
function inferLeverageStress(tqqqDrawdownPct) {
    if (tqqqDrawdownPct == null)
        return 'medium';
...
function inferRecoveryQuality(reboundStrengthPct) {
    if (reboundStrengthPct == null)
        return 'mixed';
    if (reboundStrengthPct >= 18)
        return 'strong';
    if (reboundStrengthPct >= 8)
        return 'improving';
    if (reboundStrengthPct >= 3)
        return 'mixed';
    return 'weak';
}
function attachPriorityEventVRTag(input) {
    const priorityEventTags = (0, load_priority_event_tags_1.loadPriorityEventTags)(input.rootDir);
    ...
    return {
        ...
        tag: {
            ...
            vr_analysis: {
                ...
                ma200_status: input.ma200Status === 'Sustained Below MA200' ? 'sustained_below'
                ...
                lesson: input.supportStatus === 'pending_synthetic'
...
function runPriorityEventTagExamples(rootDir) {
    const cases = [
    ...
    const tags = (0, load_priority_event_tags_1.loadPriorityEventTags)(rootDir);
    return cases.map((testCase) => ({
        event_id: testCase.eventId,
        passed: (testCase.expectedSource === 'priority_tag' && Boolean(tags[testCase.eventId])) ||
